...
(define (print-state state)
  (printf "integer: ~a~%" (get-stack  'integer state))
  (printf "float:   ~a~%" (get-stack  'float state))
  (printf "boolean: ~a~%" (get-stack  'boolean state))
  (printf "exec:    ~a~%" (get-stack  'exec state))
  (printf "code:    ~a~%" (get-stack  'code state))
  ;;(print "ticks:   " (hash-ref   state '_ticks (lambda () #f))))
  )

(define program (with-input-from-string (vector-ref (current-command-line-arguments) 0) read))

(define state (make-schush-state))
(set! evalpush-limit 10000000)
(printf "RUNNING PUSH PROGRAM ~A~%~%" program)
(time (run-schush program state))
(print-state state)
