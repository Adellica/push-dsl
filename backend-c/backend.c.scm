...
(define filenames (command-line-arguments))

;; now serialize to C!
(let ((expr (rewrite `(%begin ,@(append-map (lambda (f) (with-input-from-file f
                                          (lambda () (port-fold cons '() read))))
                                     filenames)) '() #f)))

  (with-output-to-file (conc (car filenames) ".gen.c")
    (lambda ()
      ...
      (define m-apply-literal
        `(%fun int m_apply_literal (((%pointer m_machine_t) m)
                                    (m_exec_t literal))
               ;;(printf "=== applying literal ") (write literal) (printf "\n")
      ...
      (fmt #t
           (c-expr
            `(%begin
              (%comment
               "\nautogenerated from " ,filenames " with backend.c.scm\n\n"
               ...
               ,(fmt-join/prefix dsp (-> (hash-table-keys uses)
                                 ...
                                 "\n "))
              ,nl
              (%fun int m_max ((int a) (int b)) (if (> a b) a b))
